...
export class JupyterNotebookSparkMonitor {
    ...
    createCellReactElements() {
        events.on('execute.CodeCell', (event: any, data: any) => {
            ...
            if (!cellDiv.querySelector('.sparkMonitorCellRoot')) {
                ...
                cellDiv.querySelector('.input')?.insertAdjacentElement('afterend', element);
    ...
    createButtons() {
        ...
        const full_action_name = Jupyter.actions.register(action, action_name, prefix); // returns 'my_extension:show-alert'
